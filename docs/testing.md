...
├── test_surrogate.py       # CSV + mock, features, MLP, CV / hold-out, screening split, predict_oracle, metrics CSV
├── test_qaoa.py            # Cost/mixer Hamiltonians, QAOA run, classical greedy, sweep, screening CSV
...
| `surrogate.py` | `test_surrogate.py` | CSV load (`mp_theta_sh.csv` ≥30), mock fallback, QAOA pool helpers, Mn₃Sn + real MP ID, feature matrix, training, **CV / hold-out / predict_oracle / screening split**, committed `surrogate_metrics.csv`, provenance contract |
| `qaoa.py` | `test_qaoa.py` | Cost/mixer Hamiltonian types, QAOA result structure, landscape grid, param history, classical greedy top-k, **λ/budget sweep CSV**, **screening CSV** |
...
`ruff` is configured in `pyproject.toml` under `[tool.ruff]`. CI lints `src/`. Run before committing. Regenerate NB04 surrogate artifacts with `python scripts/evaluate_surrogate.py`. Regenerate the QAOA sweep with `python scripts/run_qaoa_sweep.py` and the screening table with `python scripts/evaluate_qaoa_screening.py` (neither modifies `data/qaoa_results.csv`).
